...
function produtoLabel(p: CommodityType) {
  return PRODUTOS.find((x) => x.value === p)?.label ?? p;
}

function produtoEmoji(p: CommodityType) {
  return PRODUTOS.find((x) => x.value === p)?.emoji ?? "📦";
}
...
export default function PrecosPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8">
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-6 border-b border-field/20">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-1.5 font-mono text-xs uppercase tracking-wider px-4 py-2.5 border-b-2 transition-colors -mb-px ${
              tab === key
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        {tab === "latest" && (
          <select
            ...
            className="font-mono text-xs uppercase tracking-wider border border-field/30 bg-cream px-3 py-1.5 text-ink/70 rounded-sm focus:outline-none focus:border-field"
          >
        ...
        <button
          onClick={load}
          disabled={loading}
          className="flex items-center gap-1.5 font-mono text-xs uppercase tracking-wider border border-field/30 px-3 py-1.5 text-field hover:bg-field/5 transition-colors rounded-sm ml-auto"
        >
      ...
      {!loading && !error && records.length === 0 && (
        <div className="text-center py-16 border border-field/10 bg-field/2">
          ...
          <p className="font-mono text-sm text-ink/40 uppercase tracking-widest">
            Sem registos de preços para esta seleção
          </p>
        </div>
      )}
        ...
        <>
          {tab === "compare" ? (
            ...
            <div className="space-y-3">
              ...
              {records
                .slice()
                .sort((a, b) => Number(b.preco_kg) - Number(a.preco_kg))
                .map((r) => {
                  const pct = (Number(r.preco_kg) / maxPreco) * 100;
                  return (
                    <div key={r.id} className="flex items-center gap-3">
                      <span className="font-mono text-xs text-ink/60 w-32 shrink-0 text-right truncate">
                        {r.provincia}
                      </span>
                      <div className="flex-1 bg-field/10 rounded-sm h-7 relative overflow-hidden">
                        <div
                          className="absolute inset-y-0 left-0 bg-field/40 rounded-sm transition-all duration-700"
                          style={{ width: `${pct}%` }}
                        />
                        <span className="absolute inset-0 flex items-center px-3 font-mono text-xs font-bold text-field">
                          {fmt(r.preco_kg)} Kz
                        </span>
                      </div>
                    </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {records.map((r) => (
                <div
                  key={r.id}
                  className="border border-field/20 bg-cream p-4 hover:border-field/40 transition-colors"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <span className="text-2xl">{produtoEmoji(r.produto)}</span>
                      <p className="font-display text-sm uppercase tracking-widest text-ink mt-1">
                        {produtoLabel(r.produto)}
                      </p>
                    </div>
      ...
      <div className="mt-8 border border-field/10 bg-field/3 px-4 py-3">
        <p className="font-mono text-xs text-ink/40 uppercase tracking-wider">
          ℹ️ Preços indicativos recolhidos de mercados locais. Os valores podem variar consoante
          qualidade, época e condições de mercado.
        </p>
      </div>
    </main>
  );
