          ...
          <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {roles.map((r) => (
                  <button key={r.value} type="button" onClick={() => setRole(r.value)}
                    className={`flex flex-col items-start gap-1 p-3 border rounded-sm text-left transition-all ${
                      role === r.value ? "border-field bg-field/8 text-field" : "border-ink/15 hover:border-field/40 text-ink/60"
                    }`}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-2 sm:col-span-2">
                ...
                <input required value={nome} onChange={e => setNome(e.target.value)} placeholder="João Manuel da Silva" className="field-input rounded-sm" />
              </label>
              <label className="flex flex-col gap-2">
                <span className="label-eyebrow">Email</span>
                <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="joao@exemplo.ao" className="field-input rounded-sm" />
              </label>
              <label className="flex flex-col gap-2">
                <span className="label-eyebrow">Telefone (opcional)</span>
                <input type="tel" value={telefone} onChange={e => setTelefone(e.target.value)} placeholder="+244 9XX XXX XXX" className="field-input rounded-sm" />
              </label>
              <label className="flex flex-col gap-2">
              ...
              <label className="flex flex-col gap-2 sm:col-span-2">
                ...
                <input type="password" required minLength={6} value={senha} onChange={e => setSenha(e.target.value)} placeholder="Mínimo 6 caracteres" className="field-input rounded-sm" />
              </label>
            </div>
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full justify-center rounded-sm disabled:opacity-50">
              <UserPlus size={16} />
